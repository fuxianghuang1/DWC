...
class Fashion200k(torch.utils.data.Dataset):
    ...
    def __init__(self, path, split='train', transform=None):
        ...
        ss = pd.Categorical(all_captions)
        self.labels = ss.codes
        #self.all_captions = all_captions 
        #print('Fashion200k captions :', len(all_captions), all_captions[:100]) 
        #print('Fashion200k labels :', len(self.labels), self.labels[:100], ss)
    ...
    def generate_test_queries_(self):
        ...
        for line in lines:
            ...
            self.test_queries += [{
                ...
                'mod': {
                    #'str': target_caption
                    'str': mod_str + ' and get ' + target_caption
                    #'str': mod_str 
                }
            }]
    ...
    def __getitem__(self, idx):
        ...
        out['target_label'] =  self.labels[target_idx],
        #out['mod'] = {'str': self.imgs[target_idx]['captions'][0]}
        out['mod'] = {'str': mod_str + ' and get ' + self.imgs[target_idx]['captions'][0]}
        #out['mod'] = {'str': mod_str}
        return out
...
class FashionIQ(torch.utils.data.Dataset):
    def __init__(self, path, gallery_all=False, name = 'dress',split = 'train',transform=None):
        ...
        self.path = path
        self.image_dir = self.path + 'all_imgs'
...
class Shoes(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        out['target_img_id'] = self.imgimages_all.index(caption['target'])
