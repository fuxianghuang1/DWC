...
class Inception1d(nn.Module):
    def __init__(self, in_dim=2048, out_dim=1024):
        ...
        self.branch0 = BasicConv1d(in_dim, (out_dim//8) * 3, kernel_size=1, stride=1)

        self.branch1 = nn.Sequential(
            BasicConv1d(in_dim, out_dim//2, kernel_size=1, stride=1),
            nn.ReLU(),
            BasicConv1d(out_dim//2, out_dim//4, kernel_size=3, stride=1, padding=1)
        )
        self.branch2 = nn.Sequential(
            BasicConv1d(in_dim, out_dim//4, kernel_size=1, stride=1),
            nn.ReLU(),
            BasicConv1d(out_dim//4, out_dim//8, kernel_size=3, stride=1, padding=1),
            nn.ReLU(),
            BasicConv1d(out_dim//8, out_dim//4, kernel_size=3, stride=1, padding=1)
        )
        self.branch3 = nn.Sequential(
            nn.AvgPool1d(3, stride=1, padding=1, count_include_pad=False),
            BasicConv1d(in_dim, out_dim//8, kernel_size=1, stride=1)
        )
...
class Inception2d(nn.Module):
    ...
    def __init__(self, in_dim=2048, out_dim=2048):
        ...
        self.branch0 = BasicConv2d(in_dim, (out_dim//8) * 3, kernel_size=1, stride=1)
        self.branch1 = nn.Sequential(
